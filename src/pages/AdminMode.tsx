...
import {
  ...
  GripVertical, QrCode, KeyRound, Clock3,
} from "lucide-react";
...
function BranchSettingsManager() {
  ...
  const updateTimeSlot = (
    id: string,
    changes: Partial<InspectionTimeSlot>
  ) => {
  ...
  if (!branchInfo) {
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 text-sm text-slate-500">
        무역센터점 기존 운영 화면에서는 지점별 비밀번호·민원 채널·점검 시간 설정을 사용하지 않습니다.
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={addTimeSlot}
            className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700"
          >
            <Plus size={13} />
            시간 추가
          </button>
        ...
        <div className="rounded-xl bg-blue-50 border border-blue-100 px-3 py-3">
          <p className="text-xs text-blue-700 leading-5">
            시간대는 저장할 때 시작 시간 순으로 자동 정렬됩니다. 예를 들어
            제목을 <strong>11시</strong>, 시간을{" "}
            <strong>11:00~12:00</strong>로 설정하면 화면에는{" "}
            <strong>11시</strong>로 표시되고 11:00 이상 12:00 미만에만
            점검할 수 있습니다.
          </p>
        </div>
      </div>
